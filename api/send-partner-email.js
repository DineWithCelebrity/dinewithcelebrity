export default async function handler(req, res) {
  ...
  const { type, partnerEmail, partnerName, tier, contractDays, reason,
          voucherTitle, expiryDate, redemptions, revenue } = req.body;

  const RESEND_KEY = process.env.RESEND_API_KEY;
  if (!RESEND_KEY) return res.status(500).json({ error: 'Resend API key not configured' });
  if (!partnerEmail || !type) return res.status(400).json({ error: 'Missing partnerEmail or type' });

  const tierNames = { basic: 'Basic (Free)', growth: 'Growth (₹2,999/mo)', premium: 'Premium (₹7,999/mo)' };

  // ── unified: all handlers write to `subject` and `html` ──
  let subject = '';
  let html = '';
...
<style>
body{background:#06040A;color:#fff;font-family:Poppins,sans-serif;margin:0;padding:0}
...
.hl-key{color:rgba(255,255,255,.5)} .hl-val{color:#F2B705;font-weight:700}
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="highlight">
      <div class="hl-row"><span class="hl-key">Partner Tier</span><span class="hl-val">${tierNames[tier] || tier || 'Basic'}</span></div>
      <div class="hl-row"><span class="hl-key">Contract Duration</span><span class="hl-val">${contractDays || 180} days</span></div>
      <div class="hl-row"><span class="hl-key">Dashboard</span><span class="hl-val">partner-dashboard.html</span></div>
    </div>
    <div class="steps">
      <div class="step"><div class="step-num">1</div><div class="step-text"><strong>Sign In to Your Dashboard</strong>Use your registered email at the link below.</div></div>
      <div class="step"><div class="step-num">2</div><div class="step-text"><strong>Complete Your Profile</strong>Add outlet address, hours, and logo.</div></div>
      <div class="step"><div class="step-num">3</div><div class="step-text"><strong>Create Your First Voucher</strong>Design an offer for DWC members — live within 24 hours after review.</div></div>
      <div class="step"><div class="step-num">4</div><div class="step-text"><strong>Set Your 4-Digit PIN</strong>Used to validate member OTPs at your outlet.</div></div>
    </div>
    <a href="https://www.dinewithcelebrity.com/partner-dashboard.html" class="btn">Open Partner Dashboard →</a>
    <p style="font-size:.8rem;color:rgba(255,255,255,.5);text-align:center">Questions? Reach us at partners@dinewithcelebrity.com</p>
  </div>
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <p class="sub">We encourage you to reapply in the future as our platform grows. If you have questions, please reach out.</p>
  ...
  <div class="footer">© 2025 Dine With Celebrity Pvt Ltd · Hyderabad, India</div>
    ...
    html = `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#06040A;color:#fff;padding:32px;border-radius:16px">
  ...
  <p style="color:rgba(255,255,255,.8);font-size:.95rem;line-height:1.6">Great news! Your voucher <strong style="color:#F2B705">${voucherTitle || 'your voucher'}</strong> has been approved and is now visible to DWC premium members.</p>
  <div style="background:rgba(0,200,83,.08);border:1px solid rgba(0,200,83,.2);border-radius:12px;padding:16px;margin:20px 0">
    <div style="color:rgba(255,255,255,.6);font-size:.82rem">Members can redeem from today</div>
    <div style="color:#00C853;font-weight:700;font-size:1.1rem">Live Now</div>
  </div>
  ...
  <p style="color:rgba(255,255,255,.8);font-size:.95rem;line-height:1.6">Your voucher <strong style="color:#F2B705">${voucherTitle || 'your voucher'}</strong> requires some changes before it can go live.</p>
  ${reason ? `<div style="background:rgba(212,0,42,.08);border:1px solid rgba(212,0,42,.2);border-radius:12px;padding:16px;margin:20px 0;color:rgba(255,255,255,.8);font-size:.88rem"><strong>Reason:</strong> ${reason}</div>` : ''}
  <p style="color:rgba(255,255,255,.6);font-size:.85rem">Please update your voucher and resubmit. Our team will review within 24 hours.</p>
  <div style="text-align:center;margin-top:24px">
  ...
  <p style="color:rgba(255,255,255,.8);font-size:.95rem;line-height:1.6">Your 6-month free partnership trial ends on <strong style="color:#F2B705">${expiryDate || 'soon'}</strong>.</p>
  <div style="background:rgba(242,183,5,.06);border:1px solid rgba(242,183,5,.15);border-radius:12px;padding:16px;margin:20px 0">
    ...
    <div style="display:flex;gap:20px;flex-wrap:wrap">
      <div><div style="color:#F2B705;font-size:1.4rem;font-weight:700">${redemptions || 0}</div><div style="color:rgba(255,255,255,.5);font-size:.75rem">Redemptions</div></div>
      <div><div style="color:#00C853;font-size:1.4rem;font-weight:700">${revenue || '₹0'}</div><div style="color:rgba(255,255,255,.5);font-size:.75rem">Revenue Driven</div></div>
    </div>
  </div>
