...
// ── Pro-rata: charge difference in daily rate × remaining days ──
function calculateAmount(currentTier, newTier, expiresAt) {
  const now = new Date();

  // Free member or expired → full price, new 1-year expiry
  ...
  return {
    amount:        Math.max(proRata, 10000), // min ₹100
    isProRata:     true,
    keepExpiry:    true,                     // preserve original expiry
    remainingDays,
    originalExpiry: expiresAt,
  };
}
...
export default async function handler(req, res) {
  ...
  const { data: member } = await sbAdmin
    .from('members').select('tier, expires_at').eq('id', user.id).single();
  ...
  try {
    ...
    const order = await razorpay.orders.create({
      ...
      notes: {
        ...
        is_pro_rata:     String(pricing.isProRata),
        keep_expiry:     String(pricing.keepExpiry),
        original_expiry: pricing.originalExpiry || '',
      },
    });
    ...
    return res.status(200).json({
      ...
      user_email:      user.email,
      is_pro_rata:     pricing.isProRata,
      remaining_days:  pricing.remainingDays || null,
      original_expiry: pricing.originalExpiry || null,
    });
  } catch (err) {
